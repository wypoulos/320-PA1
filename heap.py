...
def heapify(A, i, n=None):
    ...
    if (leftChild < n and A[leftChild] < A[smallest]):
        smallest = leftChild

    if(rightChild < n and A[rightChild] < A[smallest]):
        smallest = rightChild
...
def buildHeap(A):
    ...
    the last internal node, and working backwards to the root.len(A) // 2 - 1"""
    nonLeaf = len(A) // 2 - 1
...
def heapInsert(A, v):
    ...
    A.append(v)
    parentIndex = parent(len(A) - 1)
    while(parentIndex != 0 ):
        heapify(A, parentIndex,len(A))

        if(A[parentIndex] > A[parent(parentIndex)]):
            return
